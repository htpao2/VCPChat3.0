...
const voiceService = require('../../src/services/voiceService'); // Import the new voice service
...
function initialize(mainWindow, context) {
    // Initialize the services with the same context
    chatService.initialize(context);
    voiceService.initialize(context);
    ...
    ipcMain.on('send-voice-chat-message', async (event, { agentId, history, thinkingMessageId }) => {
        ...
        try {
            const settings = await fs.readJson(path.join(APP_DATA_ROOT_IN_PROJECT, 'settings.json'));
