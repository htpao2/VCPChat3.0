...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function fetchRemoteTools() {
        try {
            ...
            // Note: In a real browser we might need to proxy this request through the main process
            // to avoid CORS if the server doesn't support it, but since we are in Electron
            // and using node-fetch in the main process proxy for tool execution, maybe we should use the same here?
            // However, renderer.js runs in renderer process.
            // Let's assume standard fetch works (if CORS allows) or use a proxy.
            // Since we added `app.use(cors({ origin: '*' }))` in server.js, standard fetch should work.
    ...
    function processRemoteTools(remoteToolsList) {
        ...
        // Strategy: Keep local tools as base (they have better UI definitions),
        // add missing remote tools.
        // Or: If a remote tool has the same name, should we overwrite?
        // Local config.js is "hand-crafted" for UI. Remote is "auto-generated".
        // Local is usually better. So we prioritize local definitions.
    ...
    function parseToolParamsFromDescription(description) {
        ...
        // Attempt to find parameter section
        // Patterns seen in descriptions:
        // "- paramName (type, required): desc"
        // "- paramName (type, optional): desc"
