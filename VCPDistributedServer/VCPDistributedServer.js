...
class DistributedServer {
    ...
    connect() {
        ...
        if (!this.mainServerUrl || !this.vcpKey) {
            // Only log if we expect to connect. If running standalone, this is fine.
...
// Self-starting logic if run directly
if (require.main === module) {
    const dotenv = require('dotenv');
    dotenv.config({ path: path.join(__dirname, 'config.env') });

    const server = new DistributedServer({
        mainServerUrl: process.env.mainServerUrl || 'http://localhost:3000',
        vcpKey: process.env.VCP_Key || 'default-key',
        serverName: process.env.SERVER_NAME || 'VCP-Distributed-Node',
        debugMode: process.env.DebugMode === 'true'
    });

    console.log('Starting VCP Distributed Server (Standalone)...');
